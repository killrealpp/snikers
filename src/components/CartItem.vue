<template>
    <div class="cart">
        <div class="cart__imgs">
            <svg
                class="cart__imgs-heart"    
                xmlns="http://www.w3.org/2000/svg"     
                width="22"  
                height="19"  
                viewBox="0 0 22 19"  
                fill="none"
                @click="cartStore.toggleFavorite(cart)"
                >
                ...
                :fill="isFavorite ? '#FF0000' : '#9B9B9B'"
                />
            </svg>
            <img class="cart__imgs-main" :src="cart.imageUrl" :alt="cart.id">
        </div>
        <div class="cart__texts">
            <h4 class="cart__title">{{ cart.title }}</h4>
            <div class="cart__sum">
                <div class="cart__price">
                    <p class="cart__price-text">Цена:</p>
                    <b class="cart__price-sum">{{ cart.price }} руб.</b>
                </div>
                <img 
                    class="cart__plus" 
...
const isInCart = computed(() => 
    cartStore.drawerList.some(item => item.id === props.cart.id)
);

const toggleCart = ()=>{
    if(isInCart.value){
        cartStore.removeDrawer(props.cart.id)
    } else {
        cartStore.addDrawer(props.cart)
    }
}

const isFavorite = computed(() => 
    cartStore.favoriteList.some(item => item.id === props.cart.id)
);
